...
export function useClient() {
  ...
  async function getAll() {
    return useApiFetch<ApiResponse<Client[]>>('/Tenant/v1/Clients/GetAll', {
      headers: tenantHeaders(),
    });
  }

  async function getById(id: number) {
    return useApiFetch<ApiResponse<Client>>(`/Tenant/v1/Clients/GetById/${id}`, {
      headers: tenantHeaders(),
    });
  }

  async function create(body: CreateClientPayload) {
    return useApiFetch<ApiResponse<Client>>('/Tenant/v1/Clients/Create', {
      method: 'POST',
      headers: tenantHeaders(),
      body,
    });
  ...
  async function update(id: number, body: UpdateClientPayload) {
    return useApiFetch<ApiResponse<Client>>(`/Tenant/v1/Clients/Update/${id}`, {
      method: 'PUT',
      headers: tenantHeaders(),
      body,
    });
